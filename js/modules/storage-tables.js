...
function attachAutoRowListener(row, tableId) {
  if (!row) return;
  const firstCell = row.cells[0];
  if (!firstCell) return;
  const primaryField = firstCell.querySelector('textarea, input:not([type="hidden"]), select');
  if (!primaryField) return;
  const triggerAutoAdd = () => {
    autoAddRow(tableId);
    saveState();
  };
  primaryField.addEventListener('input', triggerAutoAdd);
  if (primaryField.tagName === 'SELECT') {
    primaryField.addEventListener('change', triggerAutoAdd);
  }
}
...
function addRow(tableId) {
  ...
  attachAutoRowListener(row, tableId);
